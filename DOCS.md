...
-   **Gestión de Puntos de Venta (Cajas):** Se ha implementado un sistema robusto para que las empresas puedan crear múltiples "cajas" o puntos de venta. Cada caja genera un **código QR único e imprimible**, diseñado para ser descargado en formato de media carta. Esta funcionalidad utiliza `html2canvas` en el backend para renderizar un diseño profesional y estandarizado, facilitando a los clientes el inicio de un pago directo a un punto de venta específico.
