...
import type { User, ProfileSetupData, Transaction, Product, CartItem, GalleryImage, CreatePublicationInput, CreateProductInput, VerificationOutput, CashierBox, QrSession } from '@/lib/types';
...
export async function rejectUserId(userId: string) {
    await updateUser(userId, { idVerificationStatus: 'rejected', verified: false });
}
...
export async function updateCart(userId: string, productId: string, newQuantity: number) {
    ...
    if (cartTxSnap.exists()) {
        const cartItems = (cartTxSnap.data()?.details.items || []) as CartItem[];
        const itemIndex = cartItems.findIndex(item => item.product.id === productId);

        if (newQuantity <= 0) {
            if (itemIndex > -1) {
                const itemToRemove = cartItems[itemIndex];
                await updateDoc(cartTxRef, { 'details.items': FieldValue.arrayRemove(itemToRemove) });
            }
        } else {
            if (itemIndex > -1) {
                ...
                await updateDoc(cartTxRef, { 'details.items': FieldValue.arrayUnion({ product, quantity: newQuantity }) });
            }
        }
...
export async function verifyCampaignPayment(transactionId: string, campaignId?: string) {
    ...
    batch.update(doc(firestore, 'transactions', transactionId), { status: 'Pagado' });
...
export async function updateCashierBox(userId: string, boxId: string, updates: Partial<CashierBox>) {
    ...
    if(userSnap.exists()){
        ...
        const updatedBoxes = boxes.map(box => box.id === boxId ? { ...box, ...updates } : box);
...
export async function removeCashierBox(userId: string, boxId: string) {
    ...
    const userSnap = await getDoc(userRef);
     if(userSnap.exists()){
        ...
        if (boxToRemove) {
            await updateDoc(userRef, { 'profileSetupData.cashierBoxes': FieldValue.arrayRemove(boxToRemove) });
        }
    }
}
...
export async function regenerateCashierBoxQr(userId: string, boxId: string) {
    ...
    await updateCashierBox(userId, boxId, { qrValue: newQr.qrValue, qrDataURL: newQr.qrDataURL });
}
...
export async function activatePromotion(userId: string, data: { imageId: string; promotionText: string; cost: number; }) {
    const { firestore } = getFirebaseAdmin();
    const batch = writeBatch(firestore);
    ...
    batch.update(userRef, { 'promotion.text': data.promotionText, 'promotion.expires': expires.toISOString() });
    batch.update(pubRef, { 'promotion.text': data.promotionText, 'promotion.expires': expires.toISOString() });
