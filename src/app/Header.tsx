...
export default function CompanyProfilePage() {
  ...
  const { addContact, isContact, transactions, currentUser, cart, getUserMetrics, users, updateCart } = useCorabo();
  ...
  useEffect(() => {
    ...
    const loadProfileData = async () => {
        setIsLoading(true);
        try {
            const fetchedProvider = await getPublicProfile(providerId);
  ...
  const handleDirectMessage = () => {
    ...
    sendMessage({recipientId: provider.id, text: `¡Hola! Me interesa tu publicación.`, conversationId: [currentUser.id, provider.id].sort().join('-'), senderId: currentUser.id}).then(conversationId => {
  ...
  const disabledDays = Object.entries(provider.profileSetupData?.schedule || {})
    ...
    const handleConfirmAppointment = () => {
        if (appointmentDate && provider && currentUser && isCurrentUserTransactionReady && isProviderTransactionReady) {
            ...
            createAppointmentRequest({ ...request, clientId: currentUser.id });
